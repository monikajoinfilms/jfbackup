...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme756
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme756_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme756_html_head_alter(&$head_elements) {
 ...
 */
function theme756_preprocess_search_block_form(&$vars) {
...
function theme756_preprocess(&$vars, $hook) {
  $vars['theme756_path'] = base_path() . path_to_theme();
}
...
function theme756_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme756_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme756_id_safe('section-' . $section);
 ...
 */
function theme756_preprocess_page(&$vars) {
  
  //added by yogesh
	drupal_add_js(drupal_get_path('theme', 'theme756') . '/js/jquery.mousewheel-3.0.6.pack.js');
	drupal_add_js(drupal_get_path('theme', 'theme756') . '/js/fancybox/jquery.fancybox.pack.js');
	drupal_add_css(drupal_get_path('theme', 'theme756') . '/js/fancybox/jquery.fancybox.css');
 ...
 */
function theme756_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme756') . '/css/maintenance-page.css');
}
 ...
 */
function theme756_preprocess_node(&$vars) {
 ...
 */
function theme756_preprocess_block(&$vars, $hook) {
 ...
 */
function theme756_menu_tree($vars) {
 ...
 */
function theme756_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
/*function theme756_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme756_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme756_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme756_breadcrumb_showtitle')) {
 ...
 * @return bool
 */
function theme756_tabs_float() {
  $float = (bool) theme_get_setting('theme756_tabs_float');
  $float_node = (bool) theme_get_setting('theme756_tabs_node');
...
function theme756_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme756_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme756_id_safe($element['#title']);
 ...
 */
function theme756_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme756_menu_local_tasks(&$vars) {
  $output = '';
  ...
  return $output;
}/* these code add by monika*/

/**
* Override theme_breadcrumb().
*/
function theme756_breadcrumb($breadcrumb) {
  $links = array();
  $path = '';
  // Get URL arguments
